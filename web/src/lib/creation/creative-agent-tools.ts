...
const generation = object({ ref: string, mode: { type: "string", enum: ["image", "video"] }, model: string, size: string, seconds: string, quality: string, referenceRefs: strings, operation: { type: "string", enum: ["outpaint"] }, outpaint: object({ ratio: string, paddingPx: object({ left: { type: "number" }, top: { type: "number" }, right: { type: "number" }, bottom: { type: "number" } }, ["left", "top", "right", "bottom"]) }) }, ["ref", "mode", "model"]);
...
扩图（outpaint）：用户要求对一张已有图片向外扩展画幅（扩边、补背景、改成其它比例且保持原图内容不变）时，使用 operation: "outpaint"。规则：源图必须恰好引用 1 张真实图片节点（referenceRefs 指向该节点 ref）；为扩图结果创建一个新的 image 节点（prompt 写扩图后的整体画面描述），并从源图节点连边到结果节点；outpaint.ratio 填目标画幅比例（如 "16:9"、"3:4"），用户明确给出每边像素外扩量时改用 outpaint.paddingPx，两者只填其一；模型 size 档与目标比例不一致时以 ratio 为准并选择支持该方向的模型；扩图不改原图主体，提示词中不要出现"修改原图"类措辞。非扩图的普通图像编辑不使用该字段。
