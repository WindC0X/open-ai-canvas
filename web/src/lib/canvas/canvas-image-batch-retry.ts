...
export function liveImageBatchChildren(root: CanvasNodeData, nodes: CanvasNodeData[], nodeType: CanvasNodeType = CanvasNodeType.Image) {
    ...
    return nodes.filter((node) => node.id !== root.id && node.type === nodeType && node.metadata?.batchRootId === root.id);
}
...
export function reconcileImageBatchRoot(root: CanvasNodeData, nodes: CanvasNodeData[]) {
    ...
    const primary = children.find((node) => node.id === root.metadata?.[primaryField] && hasImageBatchResult(node)) || children.find(hasImageBatchResult);
