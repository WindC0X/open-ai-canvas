...
export class LineartEstimationWorker {
    ...
    private readonly pending = new Map<string, { resolve: (value: LineartWorkerSuccess) => void; reject: (error: unknown) => void; timer: ReturnType<typeof setTimeout> }>();
    // python 推理挂起/慢盘模型加载/stdout 污染任一发生时, 请求原本永久 pending 且后续请求在 stdin 排队。
    // 每个 pending 挂 watchdog: 超时 kill 重建进程并放弃全部 pending。未 loaded(含首次模型加载)5 分钟, 常规推理 2 分钟。
    ...
    async run(input: { dataUrl: string }) {
        ...
        const response = new Promise<LineartWorkerSuccess>((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                this.rejectPending(new LineartRuntimeError("lineart_runtime_timeout", "本机 AI 线稿推理超时，进程已重置", 504));
            }, this.loaded ? LineartEstimationWorker.LOADED_TIMEOUT_MS : LineartEstimationWorker.COLD_TIMEOUT_MS);
