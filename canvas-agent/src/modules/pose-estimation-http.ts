...
export class PoseEstimationWorker {
    ...
    private readonly pending = new Map<string, { resolve: (value: PoseWorkerSuccess) => void; reject: (error: unknown) => void; timer: ReturnType<typeof setTimeout> }>();
    // python 推理挂起/慢盘模型加载/stdout 污染任一发生时, 请求原本永久 pending 且后续请求在 stdin 排队,
    // 前端转圈直到杀 agent 进程。每个 pending 挂 watchdog: 超时 kill 重建进程并放弃全部 pending,
    // 让失败可见。未 loaded(含首次模型加载)给 5 分钟, 已 loaded 的常规推理给 2 分钟。
    private static readonly LOADED_TIMEOUT_MS = 120_000;
    private static readonly COLD_TIMEOUT_MS = 300_000;
    ...
    async run(input: { dataUrl: string }) {
        ...
        const response = new Promise<PoseWorkerSuccess>((resolve, reject) => {
            const timer = setTimeout(() => {
                // 超时 = worker 挂死: 放弃全部 pending 并杀进程, 下次请求由 ensureChild 重建。
                this.child = undefined;
                this.loaded = false;
                if (child && !child.killed) child.kill();
                this.rejectPending(new PoseRuntimeError("pose_runtime_timeout", "本机姿态推理超时，进程已重置", 504));
            }, this.loaded ? PoseEstimationWorker.LOADED_TIMEOUT_MS : PoseEstimationWorker.COLD_TIMEOUT_MS);
