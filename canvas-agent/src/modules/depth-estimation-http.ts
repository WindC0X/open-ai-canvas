...
export class DepthEstimationWorker {
    ...
    private readonly pending = new Map<string, { resolve: (value: DepthWorkerSuccess) => void; reject: (error: unknown) => void; timer: ReturnType<typeof setTimeout> }>();
    ...
    async run(input: { dataUrl: string }) {
        ...
        const response = new Promise<DepthWorkerSuccess>((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                this.rejectPending(new DepthRuntimeError("depth_runtime_timeout", "本机深度推理超时，进程已重置", 504));
            }, this.loaded ? DepthEstimationWorker.LOADED_TIMEOUT_MS : DepthEstimationWorker.COLD_TIMEOUT_MS);
