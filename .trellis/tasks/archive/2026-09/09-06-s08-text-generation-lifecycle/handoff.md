...
- S08 文本生成生命周期 flora 对齐（`.trellis/tasks/09-06-s08-text-generation-lifecycle/`）。工作清单四项：#15 模型列表 ✓ / #16 三个设置面板重排 ✓ / #17 composer 底部布局 ✓ / **#18 逐面验收 ✓（2026-09-11：rebase 后面 1-6 回归 + 面 7 视频/面 8 default 变体消费者(mask-edit)/面 9 亮色主题 全部真机过；过程中补修 appear 过渡态禁交互防线缺口）**。
